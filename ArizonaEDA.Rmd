...
## Load Data
```{r}
arizona <-  read_csv("~/Downloads/arizona_completed.csv")
head(arizona)
```
...
```{r}
names(arizona)[names(arizona) == 'IN THE MATTER OF'] <- "in_the_matter_of"
names(arizona)[names(arizona) == 'LICENSE TYPE'] <- "license_type"
names(arizona)[names(arizona) == 'ENFORCEMENT TYPE'] <- "enforcement_type"
names(arizona)[names(arizona) == 'FILED DATE'] <- "filed_date"
```

```{r}
p <- ggplot(data = arizona, aes(x=license_type)) +
  geom_bar(stat="count", fill="sienna4") +  coord_flip()
p
```
```{r}
p1 <- ggplot(data = arizona, aes(x=enforcement_type)) +
  geom_bar(stat="count", fill="royalblue2") +  coord_flip()
p1
```
```{r}
arizona <- arizona %>%
  mutate(filed_date = lubridate::mdy(filed_date))
```
...
```{r}
p2 <- ggplot(arizona, aes(x=filed_date)) +
...
```{r}
arizona$Text = tolower(arizona$Text)
arizona
```


```{r}
corp_az <- corpus(arizona, text_field = "Text")  # build a new corpus from the texts
summary(corp_az)
```
...
```{r}
corp_az <- corp_az %>% tokens %>% 
  tokens_remove(pattern = phrase(stop_word), valuetype = 'fixed')
```

```{r}
dfmat_az <- tokens(corp_az, remove_punct = TRUE, remove_numbers = TRUE) %>%
...
textplot_wordcloud(dfmat_az, min_count = 6, random_order = FALSE, rotation = 0.25,
...
```{r}
quant_dfm <- tokens(corp_az, remove_punct = TRUE, remove_numbers = TRUE) %>%
    tokens_remove(stopwords("en")) %>%
    dfm()
